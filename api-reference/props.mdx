...
This page provides a comprehensive reference for all props available on the `MotionIcon` component.
...
<Note>
  Icon names are case-sensitive and must match the exact Lucide icon names. Check the [Lucide icon library](https://lucide.dev/icons/) for available icons.
</Note>
